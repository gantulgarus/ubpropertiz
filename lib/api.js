...
export const getAllPosts = async (page = 1, limit = 100) => {
  const posts = await client.fetch(
    `*[_type=="post"] | order(date desc) [${(page - 1) * limit}...${
      page * limit
    }]{_id, title, date, 'slug':slug.current, content, 'featured_image':featured_image.asset->url, 'agent':agent->{name, phone, email, 'image':agent_image.asset->url }}`
  );
  return posts;
};
